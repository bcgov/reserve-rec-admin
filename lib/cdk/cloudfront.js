...
function cloudFrontSetup(scope, props) {
  ...
  const cachePolicy = new cloudfront.CachePolicy(scope, props.env.STACK_NAME + 'CachePolicy', {
    cachePolicyName: props.env.STACK_NAME + 'APIAdminCachePolicy',
  ...
  const reserveRecAdminDistribution = new cloudfront.Distribution(scope, props.env.STACK_NAME + 'Distribution', {
    enabled: true,
