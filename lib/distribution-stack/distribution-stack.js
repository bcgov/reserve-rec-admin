...
class DistributionStack extends BaseStack {
  constructor(scope, primer) {
    ...
    try {
      ...
      // Extract domain from URL (e.g., "https://api.example.com/api/" -> "api.example.com")
      this.adminApiDomain = Fn.select(2, Fn.split('/', this.adminApiUrl));
      // Extract path from URL (e.g., "https://api.example.com/api/" -> "/api")
      // Split by domain to get everything after it, then remove trailing slash
      const urlParts = Fn.split('/', this.adminApiUrl);
      this.adminApiPath = Fn.join('/', [
        '',  // Leading slash
        Fn.select(3, urlParts)  // Path segment (e.g., "api")
      ]);
