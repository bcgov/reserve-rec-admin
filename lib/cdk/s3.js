...
function s3Setup(scope, props) {
  ...
  console.log('S3_BUCKET:', props.env.S3_BUCKET);
  console.log('STACK_NAME:', props.env.STACK_NAME);

  // S3 BUCKETS

  const reserveRecAdminDistBucket = new s3.Bucket(scope, props.env.STACK_NAME + 'DistBucket', {
    bucketName: `${props.env.S3_BUCKET}`,
  ...
  // const reserveRecAdminLogBucket = new s3.Bucket(scope, props.env.STACK_NAME + 'LogBucket', {
  //   bucketName: `${props.env.S3_BUCKET}-logs`,
  //   accessControl: s3.BucketAccessControl.BUCKET_OWNER_PREFERRED,
  //   blockPublicAccess: s3.BlockPublicAccess.BLOCK_ACLS,
  //   removalPolicy: RemovalPolicy.DESTROY,
  //   autoDeleteObjects: true,
