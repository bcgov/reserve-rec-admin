...
class DistributionStack extends BaseStack {
  constructor(scope, primer) {
    ...
    // Get encryption key from SSM - should be cross-account from API app.
    try {
      logger.debug('Retrieving encryption key from SSM');
      const encryptionKeyName = resolveParameterFromSSM(this, this.getConfigValue('encryptionKeySSMPath'));
      this.encryptionKey = Key.fromKeyArn(this, this.getConstructId('encryptionKey'), encryptionKeyName);
      if (!this.encryptionKey) {
        throw new Error('Encryption key not found in SSM');
      }
    } catch (error) {
      throw new Error(`Error retrieving encryption key from SSM: ${error}`);
    }
    ...
    this.adminDistribution = new cloudfront.Distribution(this, this.getConstructId('adminDistribution'), {
      name: this.getConstructId('adminDistribution'),
      enabled: true,
      ...
      comment: 'Reserve Rec Admin CloudFront Distribution',
      compress: true,
